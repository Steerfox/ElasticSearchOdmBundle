...
* Changed the namespace of the `DocumentParserException` to `Steerfox\ElasticsearchBundle\Mapping\Exception`. #722
...
* DSL query builder was exposed to standalone [ElasticsearchDSL](https://github.com/steerfox-io/ElasticsearchDSL) library. So this effects a namespace. Run global search and replace (any modern IDE has this feature) through your project files. Change namespace `ONGR\ElasticsearchDSL\` to `ONGR\ElasticsearchDSL\`.
...
* `Manager` and `Repository` namespace is changed to `Service`, there is no more `ORM`. Again, run search and replace to find old `Steerfox\ElasticsearchBundle\ORM\` to `Steerfox\ElasticsearchBundle\Service\` namespace.
